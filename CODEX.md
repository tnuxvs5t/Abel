...
- package 目录输入会合并根项目 `src/**/*.abel`，entry 文件最后加载；依赖包会合并非 entry `src/**/*.abel` 库源码，依赖包 entry 默认排除以避免 `main` 冲突。`module/use/export` 目前只表示源码结构和意图，不提供真正 import 过滤、模块可见性、限定名查找或 export enforcement。
