...
本轮实现任务：v1.1-H H1 用户泛型/模板路线二次清扫完成：源码层 Abel 用户 template/interface/require 关键字、AST、Parser、TypeChecker、Interpreter、显式类型实参和 operator-template 路径已移除；本轮继续删除 former-template 词作为标识符的专项 parser 回归测试，并压缩文档里的历史模板流水账，避免测试矩阵和仓库手册继续携带模板路线语义包袱。
```
...
v1.1-H 状态：H1 用户泛型/模板 deep-clean 已完成；源码、测试和文档不再保留 Abel 用户模板专用入口或 former-template 兼容回归测试。H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；H3 隐式 dynamic cast 第一片已完成（init/assign/call/return/backend 参数）；H4/H6 AbelCallable / backend calls Abel func 第一片已完成；H5 dynamic binary operator 第一片已完成。下一步执行顺序是 SDK dynamic helper 深化 → backend dynamic object protocol / [] / []= → 安装版 SDK fixture 继续扩矩阵。
