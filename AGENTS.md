...
2. 下一批语言语义债建议聚焦：多文件/module source map、更多 const/引用/生命周期边界、泛型/模板/接口实际语义，以及 parser/resolver 级诊断恢复。
3. 每个后续大块仍需保持 Git 审计、4GB 测试限制、AGENTS.md 强制区更新。
4. 标准库/backend/SDK/包管理引擎应作为下一批大块推进，不要继续只做零碎局部修补。
```
...
待本次 v1-typechecker-diagnostic-decontamination 提交后追加 commit hash。
