...
本轮实现任务：AnalysisIndex 第一片。新增 `abelcore/analysis_index.h/.cpp`，定义 `AnalysisSymbolId`、`AnalysisSymbolKind`、`AnalysisBindingKind`、`AnalysisSymbol`、`AnalysisBinding`、`AnalysisExprInfo` 与 `AnalysisIndex`；`TypeChecker` 新增 `TypeCheckOptions{ collectAnalysis }` 和 `check(program, options)` opt-in API，默认 `check(program)` 不收集 analysis，保持 CLI/check/run/build 正式路径无大型 index 分配。当前 opt-in 第一片记录 top-level/struct/backend 等声明 symbol、变量 symbol、`NameExpr -> variable` binding，以及 `checkExpr` 返回的 ExprType/value category/mutability；新增 typechecker 测试证明默认无 analysis、opt-in 有 symbols/bindings/exprInfos。LSP 尚未迁移到 core AnalysisIndex，下一片再把 hover/definition/references/completion 从 AST/token 猜测逐步切到 TypeChecker 真语义。
```
...
AnalysisIndex 第一片已落地：TypeChecker 可在 opt-in 模式产出 symbols/bindings/exprInfos，默认 check 不收集，避免正式 Abel 程序运行/检查路径付 IDE 性能税。当前只记录变量 binding 与表达式类型第一片，尚未覆盖 call overload、field/method/static/backend 精确 binding。
```
