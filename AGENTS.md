...
18. 当前 std.io/std.path 仍是第一片：`scan(&x, ...)` 支持 whitespace token 输入并写入基础标量、str、any；文件/路径 builtin 支持 UTF-8 整文件读写、按行读写、路径存在/文件/目录判断和递归建目录；但还没有格式化输入、流式 IO、文件句柄对象、错误返回对象、权限/符号链接细策略、成熟 std.io module 分层或可注入输入源 API。
