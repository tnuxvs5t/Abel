...
15. 当前 const/reference 只是第一片：`const T&` 仍要求 lvalue，不做 prvalue lifetime extension；`const T*` / `T* const` 完整矩阵尚未实现；运行期对非 name lvalue（例如 const 对象字段、const vector 元素）的 const 来源识别仍不完整，后续需要继续把 constness 贯穿 location/field/index。
```
