...
当前进入 v1.2 规划/实现期：在不扩张 core TypeKind/schema 的前提下，用 anytuple/strmap dynamic literal 和 generalized pipe 把 v1.1-H 动态核心变成可编程的数据流系统。优先级从高到低：

```text
1. v1.2 dynamic runtime ADT：tuple/strmap 走 AbelDynamicObject/AbelValue，不进 TypeKind。
2. v1.2 parser/AST：[[...]] 与 [{"k" = v}]，避免和 vector init-list / index 冲突。
3. v1.2 TypeChecker：literal 返回 any；key/duplicate/boxability 检查；不追踪 schema。
4. v1.2 Interpreter：literal 构造、[]/[]=、debug/type/equality、诊断 span/stack。
5. generalized pipe：RHS 含 hole 走一般表达式 pipe context；放松多 hole，只拦同一调用多 mutable alias。
6. v1.2 complex waterworks tests：literal + pipe + dynamic operator + callback + struct/vector 组合。
7. SDK/backend 对 dynamic tuple/strmap 的消费与返回：installed fixture 覆盖。
8. v1 complete 总矩阵审计：确认 v0~v1.2 没有 parser-only 幻影和非 dynamic check/run 分裂。
9. const/pointer/reference/value-category 的 v1 最小矩阵继续收口；不做 pointer arithmetic/lifetime proof。
10. package/diagnostic 文档 smoke 继续保持，但不压过 v1.2 核心。
```
...
```text
当前阶段：v1.2 Dynamic Waterworks 第一片实现已落地；v1.2 complete 仍需 SDK/CLI/矩阵验收。
最新已知提交：不再手工固化，具体以 `git log --oneline -1` 为准。
本轮实现任务：v1.2 Dynamic Waterworks 第一片落地。新增 anytuple `[[...]]` 与 strmap `[{"k" = v}]` AST/parser/typechecker/interpreter 主路径：literal 返回 `any`，runtime 创建 `AbelDynamicObject` kind `tuple` / `strmap`，支持 `[]` / `[]=` / `any_type` / `any_debug` / dynamic equality；strmap duplicate key 在 check 阶段诊断，tuple bad index/out-of-range 与 strmap missing key 运行期稳定诊断。Pipe RHS 含 `_` 且不是旧式 callable insertion 时开始走 general expression pipe context，已支持 `_ + _`、`[[_, _ + 1]]`、`[{"k" = _}]`、`_["k"]`、`_["k"] = ...` 和 `_ = ...` lvalue 写回；旧 named/default/spread callable pipe 保持原路径。
```
...
v1.2 Dynamic Waterworks 第一片已落地：`[[...]]` / `[{"k" = v}]` 作为 any dynamic object literal 可 parse/check/run；tuple/strmap 已接入 dynamic []/[]=、debug/type、基本 equality；generalized pipe expression 第一片可让 `_` 出现在普通表达式、dynamic literal、dynamic index 和 assignment 中，支持复杂数据投影与局部水路更新。
```
...
v1.2 状态：第一片实现已通过窄测。已完成 P1~P5 的核心主路径：dynamic runtime ADT、parser/AST、TypeChecker、Interpreter、generalized pipe expression；仍需补 SDK/installed fixture、更多 complex waterworks programs、CLI/project smoke、full H10-style v1.2 completion audit。
```
