...
本轮实现任务：v1.1-H H1 template 路线二次清扫完成：源码层 Abel 用户 template/interface/require 关键字、AST、Parser、TypeChecker、Interpreter、显式类型实参和 operator-template 路径此前已移除；本轮继续删除 former-template 词作为标识符的专项 parser 回归测试，避免测试矩阵继续携带 template 路线语义包袱。当前 `template/interface/require` 只按普通文本自然落入现有词法/语法规则，不再有 Abel 用户 template 专用测试入口。
```
...
v1.1-H 状态：H1 template retired/removed deep-clean 已完成；源码与测试不再保留 Abel 用户 template 专用入口或 former-template 兼容回归测试。H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；H3 隐式 dynamic cast 第一片已完成（init/assign/call/return/backend 参数）；H4/H6 AbelCallable / backend calls Abel func 第一片已完成；H5 dynamic binary operator 第一片已完成。下一步执行顺序是 SDK dynamic helper 深化 → backend dynamic object protocol / [] / []= → 安装版 SDK fixture 继续扩矩阵。
