...
本轮实现任务：AnalysisIndex 第一片到 LSP 消费闭环。新增 `abelcore/analysis_index.h/.cpp`，定义 `AnalysisSymbolId`、`AnalysisSymbolKind`、`AnalysisBindingKind`、`AnalysisSymbol`、`AnalysisBinding`、`AnalysisExprInfo` 与 `AnalysisIndex`；`TypeChecker` 新增 `TypeCheckOptions{ collectAnalysis }` 和 `check(program, options)` opt-in API，默认 `check(program)` 不收集 analysis，保持 CLI/check/run/build 正式路径无大型 index 分配。当前 opt-in 第一片记录 top-level/struct/backend 等声明 symbol、变量 symbol、`NameExpr -> variable` binding，以及 `checkExpr` 返回的 ExprType/value category/mutability；AnalysisIndex 增加 `bindingAt` / `exprInfoAt` / `bindingsForSymbol` 查询。LSP Analyzer 已改为 opt-in 调 TypeChecker analysis，并在 hover/definition/references/documentHighlight 的局部变量路径优先使用 `Binding -> SymbolId`，旧 AST/token 逻辑保留为函数/声明 fallback。
```

### 13.1 LSP 完整化下一阶段：AnalysisIndex 工程闭环

目标：把 LSP 从 AST/token 级体验推进到真实语义体验，但不能让正式 `abel check/run/build/test` 或解释执行路径为 IDE 功能付性能税。

核心路线：

```text
TypeChecker 是唯一语义真源。
LSP 不重写第二套 lookup/overload/type/value-category 规则。
TypeChecker 在 opt-in 模式下顺手产出 AnalysisIndex。
默认 TypeChecker::check(program) 不收集 AnalysisIndex，保持正式 CLI/运行性能路径不变。
LSP 调用新的 opt-in API 获取 index，再驱动 hover/definition/references/completion/rename。
```

性能硬边界：

```text
1. 默认 check() 不分配大型 index，不记录 expr map，不生成 SymbolId。
2. 只有 LSP/测试显式请求 analysis 时启用收集。
3. Interpreter/runtime 不依赖 AnalysisIndex，不进入 run 热路径。
4. AnalysisIndex 第一片只记录必要 span/type/binding，不复制 AST，不保存大对象。
5. 任何新增记录函数都必须先判断 analysis 是否启用。
6. CLI 若未来需要 JSON analysis，必须新命令或新 flag，不改变现有 check/run 默认成本。
```

第一片最小闭环：

```text
abelcore/analysis_index.h(.cpp)
  SymbolId
  SymbolKind
  AnalysisSymbol { id, kind, name, detail, container, type, declSpan, scopeSpan }
  AnalysisBinding { useSpan, symbolId, kind }
  AnalysisExprInfo { span, type, valueCategory, mutable }
  AnalysisIndex { symbols, bindings, exprInfos, lookups by span/name }

TypeChecker
  TypeCheckOptions { collectAnalysis = false }
  TypeCheckResult { diagnostics, optional/shared AnalysisIndex }
  check(program) 保持默认无 analysis
  check(program, options) opt-in 收集

第一批记录点
  函数/方法/构造/backend/struct/field/enum/type alias/模块声明 symbol
  参数/局部变量/range-for 变量 symbol
  NameExpr -> variable/function/enum/module 等基础 binding
  checkExpr 返回处记录 ExprType
```

第二片迁移 LSP：

```text
hover 使用 AnalysisExprInfo 显示真实类型/value category。
definition/references 使用 Binding -> SymbolId，不再靠同名 token 猜。
completion 使用当前 scope 的 symbol 表，再逐步加 field/method/module members。
semantic tokens 使用 SymbolKind/ExprType，减少误染。
```
...
```text
1. 默认全量 CTest 性能路径行为不变，测试通过。
2. 新增 analysis 测试证明 opt-in 有 index，默认 check 无 index。
3. LSP 可逐步迁移，但每一片都必须保持 diagnostics 与 CLI 语义一致。
```
...
AnalysisIndex 第一/二片已落地并被 LSP 消费：TypeChecker 可在 opt-in 模式产出 symbols/bindings/exprInfos，默认 check 不收集，避免正式 Abel 程序运行/检查路径付 IDE 性能税。当前已覆盖变量、函数调用、函数值、struct constructor、method、field、backend/backend function 的 resolved binding；field/static AST 已携带 member-name span，Analysis binding 对 method/field/static/backend function 使用成员名精确 use range，避免 rename 把 receiver 一起改坏。LSP definition/references/hover 已优先消费 AnalysisIndex。LSP member completion 第一片已落地：completion 请求携带 position，`b.` / `this.` 这类简单 receiver 会用影子 dummy 文本恢复 parse/typecheck，再按 receiver struct type 返回 field/method completion；`str.` 与 `vector<T>.` 返回内建方法 completion。LSP rename/prepareRename 第一片已落地：只对 AnalysisIndex 可证明的同一 symbol 生成 WorkspaceEdit，不做 token 全局替换；声明端会重新 lex declaration span 内同名 token，避免把整段声明替换掉；prepareRename 在不可证明位置返回空。下一片应补 type/use/module/enum 精确 binding、链式/调用 receiver 成员补全、code action/inlay hints 与增量缓存。
```
...
LSP 当前是 Phase 3+ 体验边界：全量重算、AST/token 索引、基础补全、AST 大纲、diagnostics、hover/definition/workspace symbol、references/highlights、signature help、folding ranges、semantic tokens、AnalysisIndex-backed rename/prepareRename 已可用；Analyzer 已切到 TypeChecker opt-in AnalysisIndex，并覆盖变量/调用/字段/方法/构造/backend 的第一批精确 binding；简单 struct receiver 的字段/方法补全、str/vector 内建方法补全已可用。下一步是补 module/use/type/enum binding、链式/调用 receiver 成员补全、code action/inlay hints、缓存/增量/取消，继续向 CLion 级体验推进。
