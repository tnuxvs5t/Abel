...
AnalysisIndex 第一片已落地并被 LSP 消费：TypeChecker 可在 opt-in 模式产出 symbols/bindings/exprInfos，默认 check 不收集，避免正式 Abel 程序运行/检查路径付 IDE 性能税。LSP 当前只可靠消费变量 binding 与表达式类型第一片；call overload、field/method/static/backend 精确 binding 尚未覆盖。
```
...
LSP 当前是 Phase 3 体验边界：全量重算、AST/token 索引、基础补全、AST 大纲、diagnostics、hover/definition/workspace symbol、references/highlights、signature help、folding ranges、semantic tokens 已可用；下一步是把 LSP Analyzer 切到 TypeChecker opt-in AnalysisIndex，并继续补 call/field/static/backend binding，之后才能可靠推进 rename、字段/方法语义补全、表达式类型 hover 和精确 overload 体验。
