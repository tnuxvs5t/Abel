...
本轮实现任务：v1.1-H completion audit 收口。Abel 用户泛型/模板路径已审计为 clean：源码/测试不保留 Abel 层 template/interface/require token、AST、Parser 分支、TypeChecker/Interpreter 实例化路径或 former-template 兼容测试；仅剩 C++ SDK/实现自身 `template <typename ...>` helper。补强 H10 证据：explicit `cast<T>(any)` 覆盖 scalar/str/struct/func/vector 与失败诊断；installed SDK fixture 额外执行 backend dynamic object missing-key 负向运行，确认动态诊断通过安装版 CLI/plugin 闭环输出。
```
