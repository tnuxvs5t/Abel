...
2. 包管理下一批大块：`abel init` 生成项目、lockfile/resolver 骨架、local path dependency、backend artifact cache，而不是停留在 package metadata。
...
9. 当前 package manifest 只是 v1 包管理入口第一片：还没有 `abel init/add/remove/update/build`、依赖 resolver、lockfile、registry、artifact cache 或自动构建 backend plugin。
```
...
待本次 v1-package-project-entry 提交后追加 commit hash。
