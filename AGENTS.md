...
22. 当前用户自定义 operator 只是二元 overload-set 第一片：支持同一可见 operator 符号下多个二元签名候选和简单评分/歧义诊断，但不支持 `operator()` / `operator[]` / `operator<>`，不支持 unary operator overload，不支持 member operator，不支持 mutable-reference operator 参数，不支持普通函数 overload set，也不是完整 C++ overload ranking；内建 numeric/str/basic equality 仍优先于用户 operator。
```
