...
2. 包管理下一批大块：lockfile/resolver 骨架、local path dependency、backend artifact cache、`abel add/remove/update/build`，而不是停留在 package metadata/init。
...
9. 当前 package/init 只是 v1 包管理入口前两片：还没有 `abel add/remove/update/build`、依赖 resolver、lockfile、registry、artifact cache 或自动构建 backend plugin。
```
...
待本次 v1-package-init 提交后追加 commit hash。
