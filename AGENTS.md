...
本轮实现任务：v1.1-H H4/H6 AbelCallable 第一片已落地：SDK/binder 新增 `abel::AbelCallable`，backend 可从 `any`/`func` 实参接收 Abel callable 并经 Abel 调用规则回调 named function/lambda；backend 调 Abel func 的参数转换失败保留 backend frame + Abel function frame；安装版 SDK fixture 覆盖 callable roundtrip。
```
...
v1.1-H 状态：H1 template retired/removed deep-clean 已完成；H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；H3 隐式 dynamic cast 第一片已完成（init/assign/call/return/backend 参数）；H4/H6 AbelCallable / backend calls Abel func 第一片已完成。下一步执行顺序是 SDK dynamic helper 深化 → operator dynamic dispatch → backend dynamic object protocol → 安装版 SDK fixture 继续扩矩阵。
