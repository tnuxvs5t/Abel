...
本轮实现任务：v1.1-H H3/H4 cast 第一片已落地：`cast<func ...>(any)` 从 any 恢复 callable 的成功/失败路径已有端到端测试；`cast<vector<T>>(any)` 对 any 包裹的 vector 做逐元素 runtime cast，失败诊断包含元素 index，为后续隐式 dynamic cast、AbelCallable 和 backend callable 调用铺路。
```
...
v1.1-H 状态：H1 template retired deep-clean 已完成；H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；下一步执行顺序是隐式 dynamic cast 插入 → AbelCallable / backend calls Abel func → SDK dynamic helper → operator dynamic dispatch → 安装版 SDK fixture 覆盖。
