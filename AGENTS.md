...
本轮实现任务：v1.1-H H5 dynamic indexing 第一片落地：`any` receiver 的 `[]`/`[]=` 在 TypeChecker 中作为显式 dynamic boundary 接受，静态结果为 lvalue `any`；Interpreter 解包 any 后仅允许内部为 vector，index 允许 integer 或 any 包裹 integer，读写直接作用于 any 内部 vector payload。失败诊断覆盖 non-vector any 与 bad dynamic index，不新增 core map/object/tuple TypeKind。
```
...
v1.1-H 状态：H1 用户泛型/模板 deep-clean 已完成；源码、测试和文档不再保留 Abel 用户模板专用入口或 former-template 兼容回归测试。H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；H3 隐式 dynamic cast 第一片已完成（init/assign/call/return/backend 参数）；H4/H6 AbelCallable / backend calls Abel func 第一片已完成；H5 dynamic binary operator 第一片已完成；H5 dynamic indexing vector-backed 第一片已完成。下一步执行顺序是 backend dynamic object protocol / [] / []= → SDK dynamic helper 深化 → 安装版 SDK fixture 继续扩矩阵。
