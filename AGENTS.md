...
```text
1. 用户泛型/模板切除：文档删除推荐路线，清掉 Abel 层 token/parser/typechecker/interpreter/test 专用路径，不保留 retired diagnostic 兼容入口。
...
禁止保留用户泛型/模板路线或把 map/object/dict/tuple 做进核心 TypeKind。
```
...
本轮实现任务：v1.1-H H5/H6 backend dynamic object protocol 第一片落地：SDK/runtime 新增 `AbelDynamicObject`，由 backend 以 `AbelValue::makeDynamicObject(...)` 返回并通过 `any` 边界进入 Abel；TypeChecker 不新增 core TypeKind/schema，`any[key]` 静态保持 lvalue `any`。Interpreter 对 dynamic object 接入 `[]`/`[]=`/`==`/`!=` 协议，诊断保留 source span；installed SDK fixture 覆盖 backend-owned dynamic map 的 get/set/eq/debug/type。
```
...
v1.1-H 状态：H1 用户泛型/模板 deep-clean 已完成；源码、测试和文档不再保留 Abel 用户模板专用入口或 former-template 兼容回归测试。H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；H3 隐式 dynamic cast 第一片已完成（init/assign/call/return/backend 参数）；H4/H6 AbelCallable / backend calls Abel func 第一片已完成；H5 dynamic binary operator 第一片已完成；H5 dynamic indexing vector-backed 第一片已完成；H5/H6 backend dynamic object protocol 第一片已完成。下一步执行顺序是 SDK dynamic helper 深化 → dynamic diagnostic/fixture 矩阵审计 → H10 completion audit。
