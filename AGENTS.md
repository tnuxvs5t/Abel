...
2. 包管理下一批大块：`abel add/remove/build`、lockfile consumption、backend artifact cache/自动构建、registry/semver/download/cache，而不是停留在本地 path lockfile。
...
9. 当前 package/init/update 只是 v1 包管理入口前三片：resolver 只支持本地 path dependency；还没有 `abel add/remove/build`、lockfile consumption、registry、semver solver、download/cache、artifact cache 或自动构建 backend plugin。
```
...
待本次 v1-package-lockfile-resolver 提交后追加 commit hash。
