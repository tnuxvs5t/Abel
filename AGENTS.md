...
```text
当前阶段：v1.2 Dynamic Waterworks 规划已定稿；下一步进入 anytuple/strmap runtime + parser 实现。
最新已知提交：不再手工固化，具体以 `git log --oneline -1` 为准。
本轮实现任务：v1.2 Dynamic Waterworks 规划定稿。v1.2 目标固定为 anytuple `[[...]]`、strmap `[{"k" = v}]` dynamic literal 和 generalized pipe 系统：literal 第一片返回 any，lowering 到 runtime/backend-backed dynamic object，不新增 core Tuple/Map/Object TypeKind，不做 schema inference；pipe RHS 含 `_` 时升级为一般表达式 pipe context，允许 dynamic literal projection/update 与多 hole 读用法，只在同一调用边界多 mutable alias、prvalue 伪装 mutable ref 等真实风险处拒绝。下一步执行顺序：P1 dynamic runtime ADT → P2 parser/AST → P3 TypeChecker → P4 Interpreter → P5 generalized pipe → P6 SDK/complex tests。
```
...
v1.2 状态：Dynamic Waterworks 规划已定稿。anytuple `[[...]]` 与 strmap `[{"k" = v}]` 第一片固定返回 any，走 dynamic object/runtime lowering；strmap key 第一片只允许 string literal，duplicate key 是 check-time error；tuple/strmap 通过 []/[]=、any_type、any_debug、== 接入现有 dynamic protocol。pipe 将从调用层糖升级为 RHS general expression pipe context，保留无-hole callable insertion 兼容，放松多 hole 限制，只拦真实 mutable alias/lvalue 风险。
```
