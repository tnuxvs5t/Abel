...
```text
当前阶段：v1.1-H Dynamic Core Hardening scope 已完成；v1 complete 仍需最终矩阵验收。
最新已知提交：不再手工固化，具体以 `git log --oneline -1` 为准。
本轮实现任务：新增 v0~v1.1-H 大范围 corner/complex program 回归矩阵。新增 `abel_complex_program_tests` 独立测试 target，包含 16 个完整复杂正例和 8 个 corner 负例，共 24 个重测试；每个正例执行 lex/parse/module tagging/typecheck/run 全闭环，覆盖算法控制流、struct/private/public、operator overload、function value/lambda、ref/ptr/const、module/export/use alias、字符串/char/vector、named/default/pipe/spread、any/cast/dynamic operator/dynamic indexing；负例覆盖 vector element cast、dynamic operator、ref lvalue、private access、overloaded function value、pipe 多 mutable hole、dynamic index bad key、module visibility。
```
