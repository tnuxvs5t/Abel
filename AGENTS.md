...
当前补充 Abel SDK v0 范围、build-tree SDK 消费方式、backend 从 0 搭建、ResourceNode path 与排错闭环；不写入用户测试用专项 backend 示例。
...
2. 包管理下一批大块：backend artifact 自动构建、registry/semver/download/cache、安装版 SDK 与版本化缓存失效，而不是停留在本地 path dependency、lockfile consumption 与缓存复制。
3. 语言语义下一批：多文件/module source map、更多 const/引用/生命周期边界、泛型/模板/接口实际语义，以及 parser/resolver 级诊断恢复。
4. 每个后续大块仍需保持 Git 审计、4GB 测试限制、AGENTS.md 强制区更新。
5. 标准库/backend/SDK/包管理引擎应作为下一批大块推进，不要继续只做零碎局部修补。
```
...
9. 当前 package/init/update/add/remove/build/graph/cache 只是 v1 包管理入口前七片：resolver 只支持本地 path dependency，cache 只是复制已存在的 plugin artifact；还没有 registry、semver solver、download cache、自动构建 backend plugin、安装版 SDK 或版本化缓存失效。
```
...
待本次 v1-backend-artifact-cache 提交后追加 commit hash。
