...
172. README / TUTORIAL_zh.md / CODEX.md 同步 `abel build` 当前边界：它是早期项目 build 门面，不假装已经具备 backend artifact 自动构建/cache。
...
2. 包管理下一批大块：backend artifact 自动构建、registry/semver/download/cache、安装版 SDK 成熟化与版本化缓存失效，而不是停留在本地 path dependency、lockfile consumption、缓存复制与 SDK 第一片。
3. Backend/SDK 下一批大块：variadic any backend binder、明确 pointer/reference 暴露矩阵、plugin 自动构建、ABI/Qt kit 缓存失效策略。
4. 语言语义下一批：多文件/module source map、更多 const/引用/生命周期边界、泛型/模板/接口实际语义，以及 parser/resolver 级诊断恢复。
...
5. v1 source location 已开始贯穿 AST、runtime frame、backend call context；后续仍需覆盖多文件/module source map、`__FILE__` / `__LINE__` / `__COLUMN__` 内建和更完整诊断恢复。
6. 零参 `init()` 被视为默认构造入口；字段初始 raw default 后再执行 constructor body。若未来开放函数字段/更复杂不可默认字段，需要补更细的字段初始化证明与诊断。
7. 当前 `__FILE__` / `__LINE__` / `__COLUMN__` 来源为单文件 AST SourceSpan；多文件/module/use 体系完成前，它们只能保证当前解析文件内的调用点位置。
8. 当前诊断去污染第一块主要覆盖 TypeChecker 表达式/语句级 unknown 传播；parser/resolver、缺 return 分析、多文件符号恢复仍需后续大块继续做。
9. 当前 package/init/update/add/remove/build/graph/cache/install-sdk 只是 v1 包管理与 backend SDK 入口前几片：resolver 只支持本地 path dependency，cache 只是复制已存在的 plugin artifact，安装版 SDK 只是第一片；还没有 registry、semver solver、download cache、自动构建 backend plugin、完整 SDK 成熟化或版本化缓存失效。
10. 当前 backend binder 已覆盖常用 scalar/vector/诊断通道，但仍不承诺任意 struct/class 自动拆装箱、任意 pointer/reference 矩阵、任意 T& 写回、variadic any... binder 或跨 Qt/编译器 ABI 稳定；`AbelRuntimeContext&` 仅支持作为 C++ lambda 最后一个参数。
```
...
待本次 v1-backend-binder-matrix 提交后追加 commit hash。
