...
本轮实现任务：继续 v1.1-a Structured Calls 的 pipe holes 收口；在函数/函数值/builtin function holes 之后，补齐 `_.method()` / `_.field.method()` receiver 形态的 TypeChecker/Interpreter 闭环，并保持 lhs 只求值一次。
```
...
v1.1-a Structured Calls 第一片已启动：`x |> f(_)`、`x |> f(1, _)`、`x |> f(_, _)` 对普通函数、函数值和 builtin function 可用；`x |> _.method()`、`x |> _.field.method()` 对单 receiver hole 可用；lhs 运行期只求值一次，hole 保留 lvalue 以绑定 `T&`；若同一个 pipe hole 会绑定多个 mutable reference 参数，TypeChecker 拒绝。当前尚未扩展到 named/default args 或 limited spread。
```
...
v1.1-a 剩余：named/default args、limited spread into any...、backend/constructor 路径的 hole 归一化仍未完成；receiver-hole 第一片当前限制为单 `_`，尚未开放 `_.method(_)` 这类多 hole / mutable receiver 组合；任何后续推进都必须继续走 TypeChecker/Interpreter/测试三件套闭环。
```
