...
本轮实现任务：Abel VSCode LSP Phase 1。新增 `abel-lsp` 可执行文件与 `abellspcore`，通过 stdio JSON-RPC 支持 initialize/shutdown、didOpen/didChange/didSave/didClose、publishDiagnostics、documentSymbol、基础 keyword/snippet completion；LSP Analyzer 复用 Lexer/Parser/TypeChecker，并按 CLI 多文件 package 规则处理 module/use/export use/import alias 与 open document overlay；新增 `vscode/abel-vscode` 最小 VSCode client 与基础 TextMate grammar，支持 `.abel` 文件启动 `abel-lsp` 和基础高亮；新增 `abel_lsp_tests` 覆盖 span->LSP range、open document diagnostics/symbols、package 多文件分析。
```
...
VSCode LSP Phase 1 已落地：`abel-lsp` 提供 diagnostics + outline/document symbols + cheap completion 的第一阶段可用闭环；VSCode 扩展为薄客户端，通过 `abel.lsp.path` 指向 server。当前不提供 hover、go to definition、references、rename、semantic tokens 或增量语义分析；这些等待后续 SemanticIndex。
```
...
LSP 当前是 Phase 1 边界：全量重算、基础补全、AST 大纲和 diagnostics 已可用；hover/definition/references/rename/semantic completion 需要先抽 SemanticIndex，不在第一阶段内。
