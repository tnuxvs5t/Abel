...
2. 包管理下一批大块：lockfile consumption、backend artifact cache/自动构建、registry/semver/download/cache，而不是停留在本地 path dependency 与 build 门面。
...
9. 当前 package/init/update/add/remove/build 只是 v1 包管理入口前五片：resolver 只支持本地 path dependency，build 只是项目级检查门面；还没有 lockfile consumption、registry、semver solver、download/cache、artifact cache 或自动构建 backend plugin。
```
...
待本次 v1-package-build-command 提交后追加 commit hash。
