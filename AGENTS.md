...
VSCode LSP Phase 1/2/3 体验闭环已落地：`abel-lsp` 提供 diagnostics、outline/document symbols、keyword+symbol completion、hover、同名声明 definition、workspace symbol、references、documentHighlight、signatureHelp、foldingRange；VSCode 扩展为薄客户端，通过 `abel.lsp.path` 指向 server。当前 SemanticIndex 是 AST/token 级索引，不是完整 name-binding/type index，因此 rename、字段/方法语义补全、semantic tokens、表达式类型 hover、精确 overload binding 和增量语义分析仍未做。
```
...
LSP 当前是 Phase 3 体验边界：全量重算、AST/token 索引、基础补全、AST 大纲、diagnostics、hover/definition/workspace symbol、references/highlights、signature help、folding ranges 已可用；下一严重难点是让 TypeChecker 输出真实 binding/expr-type index，之后才能可靠推进 rename、字段/方法语义补全、semantic tokens、表达式类型 hover 和精确 overload 体验。
