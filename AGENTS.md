...
2. 下一批语言语义债建议聚焦：vector<struct>.resize 默认构造/default-constructible、诊断去污染、`__FILE__` / `__LINE__` / `__COLUMN__` 或等价 source_location 内建。
3. 每个后续大块仍需保持 Git 审计、4GB 测试限制、AGENTS.md 强制区更新。
4. 发布后若需要更强访问控制，应将 GitHub 仓库改 private；若保持 public，则依靠 LICENSE/著作权声明保护使用权边界。
```
...
待本次 v1-runtime-stack-source-location 提交后追加 commit hash。
