...
本轮实现任务：v1.1-H H1 已落地：清掉 Abel 用户 template 的 AST / Parser 成功入口 / TypeChecker 实例化推导 / Interpreter 运行路径，保留 retired/reserved 诊断，为后续 any-based typecheck 重做打干净基础。
```
...
历史无约束 template 第一片曾落地，但新路线已决定 template 全路线 retired；当前已清掉 Abel 用户 template 的 AST 字段、TypeChecker 推导/实例化路径、Interpreter 运行路径和成功测试，只保留 Parser retired/reserved 诊断与普通尖括号语法 `vector<T>` / `func` / `cast<T>`。
...
template v1 最简无约束函数/struct/type alias 与 exact-shape 二元 operator template 历史上已落地，但现在全路线 retired；当前已完成实现清理与测试迁移，Parser 对 `template <type ...>` 给稳定 retired/reserved 诊断，TypeChecker/Interpreter 不再保留 template bindings / instantiation / exact-shape operator template 逻辑。
...
v1.1-H 状态：H1 template retired 清理已完成；下一步执行顺序是 any 动态核心 → cast/隐式 dynamic cast → func-as-any / AbelCallable → SDK dynamic helper → operator dynamic dispatch → 安装版 SDK fixture 覆盖。
