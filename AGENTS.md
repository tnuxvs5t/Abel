...
2. 包管理下一批大块：`abel build`、lockfile consumption、backend artifact cache/自动构建、registry/semver/download/cache，而不是停留在本地 path dependency 命令面。
...
9. 当前 package/init/update/add/remove 只是 v1 包管理入口前四片：resolver 只支持本地 path dependency；还没有 `abel build`、lockfile consumption、registry、semver solver、download/cache、artifact cache 或自动构建 backend plugin。
```
...
待本次 v1-package-add-remove 提交后追加 commit hash。
