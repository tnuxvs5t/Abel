...
348. BuiltinRegistry 新增 std.io/std.path 文件路径第一片：`read_text`、`write_text`、`read_lines`、`write_lines`、`path_exists`、`path_is_file`、`path_is_dir`、`mkdirs`。
349. TypeChecker 增加文件路径 builtin 静态规则：路径参数必须是 `str`，`write_text` 内容必须是 `str`，`write_lines` 第二参必须是 `vector<str>`，direct call 与 pipe call 返回类型保持一致。
350. Interpreter 通过 BuiltinRegistry 执行 UTF-8 文本读写、按行读写、路径判断与递归建目录；运行期文件打开/写入/建目录失败产生结构化诊断。
351. builtin/typechecker/interpreter 回归覆盖文件路径 builtin 注册、成功读写、pipe 形态、静态误用拒绝和运行期文件错误诊断；README / TUTORIAL_zh.md / CODEX.md 同步普通文件路径能力不再必须 backend。
352. BuiltinRegistry 新增 std.io/std.path/std.env 第二片：`append_text`、`copy_file`、`move_path`、`remove_path`、`path_join`、`path_dirname`、`path_basename`、`path_ext`、`path_absolute`、`path_clean`、`current_dir`、`env_exists`、`env_get`。
353. TypeChecker 增加第二片文件/路径/环境 builtin 静态规则：追加/复制/移动/path_join 的第二参数必须是 `str`，`current_dir()` 必须零参，路径查询/环境查询返回类型和 direct/pipe 调用保持一致。
354. Interpreter 通过 BuiltinRegistry 执行 UTF-8 追加写入、文件复制、路径移动/删除、路径拼接拆分、当前目录查询和环境变量读取；运行期失败产生 E044x 结构化诊断。
355. builtin/typechecker/interpreter 回归覆盖第二片文件/路径/环境 builtin 注册、运行结果、pipe 形态和静态误用拒绝；README / TUTORIAL_zh.md / CODEX.md 同步普通文件、路径、环境能力边界。
356. BuiltinRegistry 新增 std.char 第一片：`char_code`、`char_from_code`、`char_is_digit`、`char_is_letter`、`char_is_alnum`、`char_is_space`、`char_is_upper`、`char_is_lower`、`char_upper`、`char_lower`、`char_to_str`。
357. BuiltinRegistry 新增 std.any 第一片：`any_type`、`any_is`、`any_is_bool`、`any_is_int`、`any_is_double`、`any_is_char`、`any_is_str`、`any_is_vector`、`any_is_pointer`；不提供 `any_unwrap` 动态逃逸口，避免破坏 check/run 类型一致性。
358. TypeChecker 增加 std.char/std.any direct 与 pipe 静态规则；builtin/typechecker/interpreter 回归覆盖注册、运行结果、pipe 形态、静态误用拒绝与 `char_from_code` 越界运行期诊断；README / TUTORIAL_zh.md / CODEX.md 同步 std.char/std.any 使用边界。
359. ResourceNode 新增 `platform`、`compiler`、`compilerVersion`、`cxxStandard`、`abelAbi` 字段与当前 runtime 查询函数；load 阶段在 QPluginLoader 前逐项检查 Qt version、kit、platform、compiler、compiler version、C++ standard 与 Abel ABI。
360. Package backendArtifacts 对新增兼容字段自动填当前 runtime 默认值，普通用户继续只需写 backendId/path/symbols/build；backend artifact cache sidecar 升级到 formatVersion 2，并把平台/编译器/C++/Abel ABI 字段纳入 stale 判定。
361. backend/package 回归覆盖专家 ResourceNode JSON 新字段、foreign compatibility 字符串 parse 不拒绝但 load 拒绝、package manifest 默认填充、cache metadata 写入与 compatibility metadata stale 时回退源 artifact；README / TUTORIAL_zh.md / CODEX.md 同步 ResourceNode 专家字段与普通 package 路径。
362. Parser consume/recovery 规则从“缺失 token 时直接吞下一个 token”改为按预期 token 与发现 token 决定是否合成缺失 token；缺分号、缺右括号、缺右花括号等高频错误不再破坏后续 AST 主干。
363. Parser block-level recovery 增加声明/语句同步点：普通 block 遇到疑似顶层声明会停止并把控制权交回顶层 parser，backend/struct 成员场景仍保留合法 `fn` 成员解析。
364. 增加 parser/typechecker 回归：缺语句分号后保留 var decl 与 return；未闭合函数体后保留下一个顶层 `main`；parser 根因不会继续追加 missing-return 噪音。
```
...
9. 当前 package/init/update/add/remove/build/package publish/registry index/graph/cache/install-sdk 只是 v1 包管理与 backend SDK 入口前几片：resolver 支持本地 path dependency、本地 registry dependency、SemVer requirement 第一片、同名包多解析冲突诊断第一片与 `.abel-registry.json` 索引消费第一片，registry publish/cache/index 只是把本地目录包复制到 registry、生成/消费 `.abel-registry.json` 或复制到 `.abel/cache/packages`，backend artifact 自动构建只支持 CMake 第一片，安装版 SDK 只是第一片；backend cache sidecar 已覆盖源 artifact 路径/size/mtime、ResourceNode 字段、platform/compiler/C++/Abel ABI 字符串与 symbols 匹配，但还没有远程 registry、完整 semver solver、网络 download cache、完整 SDK 成熟化、二进制内容 ABI hash/semver 级校验或完整版本化缓存失效。
