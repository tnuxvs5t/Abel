...
本轮实现任务：Abel VSCode LSP Phase 2 第一片。新增轻量 AST SemanticIndex：Analyzer 在 package/single-file 分析后收集 module/use/fn/struct/field/init/method/enum/type alias/backend/backend fn 等声明符号，生成 detail/container/location；LSP server 宣告并实现 `textDocument/hover`、`textDocument/definition`、`workspace/symbol`。当前定义跳转按光标 token 到同名声明集合，hover 优先 token 精确命中再退回声明范围；这不是完整 TypeChecker binding graph，但已经解锁日常查看签名、跨文件跳声明和工作区符号搜索。
```
...
VSCode LSP Phase 1/2 第一片已落地：`abel-lsp` 提供 diagnostics、outline/document symbols、cheap completion、hover、同名声明 definition、workspace symbol 的可用闭环；VSCode 扩展为薄客户端，通过 `abel.lsp.path` 指向 server。当前 SemanticIndex 是 AST 声明索引，不是完整 name-binding/type index，因此 references、rename、字段/方法语义补全、semantic tokens、表达式类型 hover 和增量语义分析仍未做。
```
...
LSP 当前是 Phase 2 第一片边界：全量重算、基础补全、AST 大纲、diagnostics、AST 符号 hover/definition/workspace symbol 已可用；下一高杠杆点是让 TypeChecker 输出真实 binding/expr-type index，之后再做 references/rename/semantic completion/semantic tokens。
