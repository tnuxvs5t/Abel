...
### 11.1 v1 普通函数 overload-set 第一片

v1 已开始落地普通函数重载，但当前是顶层普通函数的可测子集，不是完整 C++ overload system。
...
```text
同一 package/module 下允许同名普通顶层函数拥有不同参数类型签名。
同一 package/module 下同名且参数类型签名完全相同视为 duplicate。
不按返回类型重载；返回类型不参与候选选择。
direct call、pipe call 与 module-qualified call 都走 overload-set 选择。
候选仍受 package/module/use/export 可见性约束；限定调用不绕过 use。
TypeChecker 与 Interpreter 共享第一片简单评分：
  - 精确值参数优先；
  - 其次 const/reference 精确绑定；
  - 再其次可转换参数；
  - variadic any... 作为较弱候选；
  - 同分多个候选时报 ambiguous；
  - 无候选匹配时报 no matching overload。
普通单签名函数仍保留原有运行期转换诊断路径，避免把实参转换错误误报为 overload 不匹配。
```

当前明确不支持：

```text
backend function overload。
返回类型 overload。
默认参数。
...
struct 方法与构造 overload 已作为独立第一片推进，见下一节；它不属于普通顶层函数 overload-set。

### 11.2 v1 struct constructor / method overload-set 第一片

v1 已开始落地 struct 构造与方法重载，但当前仍是可测子集，不是完整 C++ overload system。
...
```text
同一个 struct 内允许多个 init(...) 构造，只要参数类型签名不同。
同一个 struct 内允许同名方法拥有不同参数类型签名。
同名方法 duplicate 判定包含 const receiver 标记与参数类型签名；完全相同则拒绝。
构造 duplicate 判定只看参数类型签名；完全相同则拒绝。
无显式 init 的 struct 继续保留旧 positional field construction 与默认构造规则。
只有一个显式 init / 单个方法候选时，保留旧的参数转换诊断路径。
多个候选时，TypeChecker 与 Interpreter 使用同一套第一片简单评分：
  - 精确值参数优先；
  - 其次 const/reference 精确绑定；
  - 再其次可转换参数；
  - variadic any... 作为较弱候选；
  - 同分多个候选时报 ambiguous；
  - 无候选匹配时报 no matching overload。
Interpreter 在 overload 选择前只求值一次实参，同时保留 value/location/readonly/span；
选中 T& 参数后绑定真实 location，避免构造/方法重载导致引用参数写回或副作用求值次数分裂。
private 构造/方法仍受当前 struct 上下文约束。
const receiver 不能调用非 const 方法；mutable receiver 优先选择非 const 候选。
```
...
模板方法 / 模板构造。
完整 C++ overload ranking。
prvalue struct 临时对象方法调用的完整生命周期模型。
```
...
### 15.1 v1 用户自定义二元 operator 第一片

v1 已开始落地用户自定义 operator，但当前是可测子集，不是完整 C++ overload system。
...
```text
operator 声明本质是普通顶层函数，内部函数名归一为 `operator <symbol>`。
每个 operator overload 必须恰好两个参数。
不能 variadic。
不能 debt。
当前切片禁止 mutable reference 参数；const/reference 更完整矩阵后续推进。
内建 numeric、str 拼接、基础 bool/char/str/pointer/nullptr equality 优先于用户 operator。
非内建 operand 或无内建比较时才查找用户 operator。
TypeChecker 与 Interpreter 都用当前 package/module/use 可见性解析 operator 函数。
同一可见 operator 符号支持多个二元签名候选。
候选选择第一片使用简单分数：精确值参数优先，其次 const reference 精确绑定，其次可转换参数。
同分多个候选时报 ambiguous。
同一 package/module 下相同 operator 参数签名视为 duplicate。
```
...
member operator
完整 C++ overload ranking
普通函数 overload-set 已作为独立 v1 第一片支持；不属于 operator 系统。
用户 operator 覆盖内建 numeric 行为
```
...
当前已落地 v1 package local registry publish 第一片：`abel package publish [--overwrite] <project-dir> <registry-dir>` 会把包复制到本地 registry 的 `<name>/<version>`，跳过 `.abel` 缓存，并可被 `abel add registry` / `abel run` 消费。
当前已落地 v1 package local registry index 第一片：发布时刷新 `.abel-registry.json`，`abel package registry index/check/list <registry-dir>` 可重建、校验与查看本地 registry 包版本。
当前已落地 v1 package indexed registry resolution 第一片：registry dependency resolver 在 `.abel-registry.json` 存在时会校验并消费索引，索引 stale/malformed 时拒绝解析；无索引的旧本地 registry 才退回目录扫描。
当前已落地 v1 `file://` registry mirror-cache 第一片：registry dependency 的 `registry` 字段和 `abel add registry` 可接收本机 `file://` URI，resolver 先镜像到 `.abel/cache/registries/...`，再复用 indexed registry / SemVer / `.abel/cache/packages` 闭环。
...
当前已落地 v1 普通函数 overload-set 第一片：同一 package/module 下普通顶层函数可按参数类型多签名共存，direct/pipe/module-qualified call 共享候选选择；TypeChecker 与 Interpreter 同步支持 duplicate signature、no matching overload 与 ambiguous 诊断。
...
当前已落地 v1 `std.test` 第二片：`test_close(actual, expected, eps, any... msg)` 进入 BuiltinRegistry/TypeChecker/Interpreter，浮点近似断言复用 E0599、runtime stack 与 source excerpt/caret 诊断通道。
当前已落地 v1 `abel test` 第二片：`abel test --filter <substring> [project-dir]` 可按测试文件相对路径筛选测试，项目测试入口具备基础筛选能力。
当前已落地 v1 `abel test` report 第一片：`abel test --report-json <file> [project-dir]` 可输出机器可读测试报告，记录项目、filter、汇总计数、逐测试 status/phase/exitCode 与 diagnostic JSON。
当前已落地 v1 `abel test` JUnit report 第一片：`abel test --report-junit <file> [project-dir]` 可输出 CI 常用 JUnit XML，记录 testsuite/testcase/failure 与 filtered test 集合。
当前已落地 v1 `abel test` expect-fail 第一片：`abel test --expect-fail <substring> [project-dir]` 可按测试相对路径标记预期失败，失败记为 `xfail` 且不失败，意外通过记为 `xpass` 且让命令失败，JSON/JUnit report 均保留状态。
当前已落地 v1 `abel test` fixture lifecycle 第一片：测试文件可声明 `fn void setup()` / `fn void teardown()`，测试入口自动在 `main` 前后调用，`teardown` 在 setup 失败或 main 失败后仍会执行。
...
当前已落地 v1 std.str 扩展方法第一片：`starts_with/ends_with/trim/lower/upper/split` 进入 BuiltinRegistry/TypeChecker/Interpreter，字符串常用处理能力与 check/run 规则保持一致。
当前已落地 v1 std.str 第二片：`str.join(vector<str>)` 与 `str.parse_int/parse_long/parse_double/parse_bool()` 进入 BuiltinRegistry/TypeChecker/Interpreter，字符串拼接与基础解析能力保持 check/run 一致。
...
当前已落地 v1 std.vec builtin algorithms 第二片：`vector.reverse/unique/binary_search/lower_bound/upper_bound` 进入 BuiltinRegistry/TypeChecker/Interpreter，容器常用算法与 check/run 规则保持一致。
当前已落地 v1 std.vec 第三片：`vector.contains/count/extend/slice` 进入 BuiltinRegistry/TypeChecker/Interpreter，常用成员查询、批量追加与切片复制能力保持 check/run 一致。
...
当前已落地 v1 std.io/std.path 文件路径第一片：`read_text/write_text/read_lines/write_lines/path_exists/path_is_file/path_is_dir/mkdirs` 进入 BuiltinRegistry/TypeChecker/Interpreter，UTF-8 文本读写、按行读写、路径判断、递归建目录与 pipe 形态保持 check/run 一致。
当前已落地 v1 std.io/std.path/std.env 第二片：`append_text/copy_file/move_path/remove_path/path_join/path_dirname/path_basename/path_ext/path_absolute/path_clean/current_dir/env_exists/env_get` 进入 BuiltinRegistry/TypeChecker/Interpreter，常用文件追加、复制、移动、删除、路径拆装、当前目录与环境变量读取保持 check/run 一致。
当前已落地 v1 std.math builtin 第一片：`abs/sqrt/floor/ceil/round/trunc/pow/min/max/clamp` 进入 BuiltinRegistry/TypeChecker/Interpreter，基础数值工具的参数数目、numeric-only 静态规则、pipe 形态与运行期 bad clamp bounds 诊断保持 check/run 一致。
当前已落地 v1 std.math 第二片：`sin/cos/tan/asin/acos/atan/atan2/exp/log/log10/gcd/lcm` 进入 BuiltinRegistry/TypeChecker/Interpreter，三角/指数/对数统一返回 f64，`gcd/lcm` 静态与运行时都要求整数参数。
当前已落地 v1 std.char/std.any 第一片：`char_code/char_from_code/char_is_digit/char_is_letter/char_is_alnum/char_is_space/char_is_upper/char_is_lower/char_upper/char_lower/char_to_str` 与 `any_type/any_is/any_is_bool/any_is_int/any_is_double/any_is_char/any_is_str/any_is_vector/any_is_pointer` 进入 BuiltinRegistry/TypeChecker/Interpreter；direct call 与 pipe call 共享静态签名，`any` helper 仅用于检查，真实取值仍由 `cast<T>(any)` 完成。
当前已落地 v1 backend compatibility metadata 第一片：ResourceNode / package backendArtifacts / backend cache sidecar 统一携带 `platform`、`compiler`、`compilerVersion`、`cxxStandard`、`abelAbi`，package manifest 对普通用户自动填当前 runtime，专家 ResourceNode JSON 必须显式声明；加载期会在 QPluginLoader 前拒绝平台、编译器、C++ standard 或 Abel ABI 不匹配的 plugin，缓存 sidecar 不匹配时回退源 artifact。
当前已落地 v1 Parser 诊断恢复第一片：缺失语句分号不会吞掉下一条 `return`/语句，未闭合普通 block 会同步到下一条顶层声明，parser 根因不再继续污染为误导性 missing-return。
当前已落地 v1 用户自定义二元 operator 第一片：`fn R operator +(A a, B b) { ... }` 这类普通二元 operator 函数可被 Parser/TypeChecker/Interpreter 识别和调用，覆盖 `+ - * / % %% ** <? >? == != < <= > >=` 的可测子集；内建 numeric/str/基础比较仍优先，`operator()` / `operator[]` / `operator<>` 仍明确排除。
当前已落地 v1 用户自定义二元 operator overload-set 第一片：同一可见 operator 符号可有多个二元签名，check/run 共享简单候选评分与歧义诊断；同一 package/module 下相同 operator 签名被拒绝。
当前已落地 v1 struct constructor/method overload-set 第一片：同一个 struct 内支持多签名 `init(...)` 与同名方法 overload，TypeChecker 与 Interpreter 共享简单候选评分、duplicate/no-match/ambiguous 诊断、private/const receiver 约束，并在运行期 overload 选择前只求值一次实参。
```
...
308. BuiltinRegistry 新增 std.math 第一片：`abs/sqrt/floor/ceil/round/trunc/pow/min/max/clamp` 覆盖整数/f64 常用数值工具，`sqrt/floor/ceil/round/trunc/pow` 返回 f64，`abs/min/max/clamp` 保持或合成数值结果类型。
309. TypeChecker 增加 std.math 静态规则：参数个数、numeric-only、direct call 与 pipe call 返回类型共享规则；非 numeric 参数和错误 arity 在 check 阶段拒绝。
310. builtin/typechecker/interpreter 回归覆盖 std.math 注册、运行结果、pipe 形态、静态误用拒绝与 `clamp(low > high)` 运行期诊断。
311. BuiltinRegistry 新增 std.str 扩展方法：`starts_with/ends_with/trim/lower/upper/split`。
312. TypeChecker 增加 std.str 扩展方法静态规则：参数个数、str 参数与 `vector<str>` 返回。
313. builtin/typechecker/interpreter 回归覆盖字符串前后缀、trim、大小写转换、split 与错误参数。
314. BuiltinRegistry 新增 std.math 第二片：`sin/cos/tan/asin/acos/atan/atan2/exp/log/log10/gcd/lcm`。
315. TypeChecker 增加 std.math 第二片静态规则：三角/指数/对数 numeric-only 且返回 f64，`gcd/lcm` direct 与 pipe 形态均要求 integer。
316. builtin/typechecker/interpreter 回归覆盖 std.math 第二片注册、运行结果、pipe 形态与错误参数。
317. BuiltinRegistry 新增 std.vec 第二片：`reverse`、`unique`、`binary_search`、`lower_bound`、`upper_bound`。
318. TypeChecker 增加 std.vec 第二片静态规则：mutating receiver、元素参数、orderable 元素与返回类型。
319. builtin/typechecker/interpreter 回归覆盖 reverse、unique、binary_search、lower_bound、upper_bound 与错误参数。
320. BuiltinRegistry 新增 std.str 第二片：`join`、`parse_int`、`parse_long`、`parse_double`、`parse_bool`。
321. TypeChecker 增加 std.str 第二片静态规则：join 参数必须是 `vector<str>`，parse 系列零参并返回精确类型。
322. builtin/typechecker/interpreter 回归覆盖字符串 join、基础解析、错误参数与坏解析输入运行期诊断。
323. BuiltinRegistry 新增 std.vec 第三片：`contains`、`count`、`extend`、`slice`。
324. TypeChecker 增加 std.vec 第三片静态规则：元素参数、同类型 vector extend、slice 整数边界与 mutable receiver。
325. builtin/typechecker/interpreter 回归覆盖 contains、count、extend、slice、错误参数与运行结果。
326. BuiltinRegistry 新增 `std.test` 第二片：`test_close(actual, expected, eps, any... msg)`。
327. TypeChecker 增加 `test_close` 静态规则：actual/expected/eps 必须 numeric，消息参数必须可 stringify，并支持 pipe 形态。
328. builtin/typechecker/interpreter 回归覆盖 `test_close` 通过、失败诊断、错误参数和 source excerpt/call stack。
329. CLI `abel test` 新增 `--filter <substring>`，按测试文件相对路径筛选运行集合。
330. CLI 多文件项目回归覆盖 filter 只运行匹配测试、跳过不匹配失败测试与 filtered summary。
331. CLI `abel test` 新增 `--report-json <file>`，输出机器可读项目测试报告，包含项目、filter、汇总计数和逐测试结果。
332. CLI 多文件项目回归覆盖 JSON report 写入、filtered report 不包含未匹配测试、失败测试 report 记录 failed status / run phase / exitCode。
333. CLI `abel test` 新增 `--report-junit <file>`，输出 CI 常用 JUnit XML，包含 testsuite/testcase/failure 基础结构。
334. CLI 多文件项目回归覆盖 filtered JUnit report、失败 JUnit report、failure 元素和失败 exit code。
335. CLI `abel test` 新增可重复 `--expect-fail <substring>`，按测试文件相对路径标记预期失败。
336. CLI 多文件项目回归覆盖 xfail 不导致命令失败、xpass 导致命令失败、JSON/JUnit report 记录 xfail/xpass 与汇总计数。
337. CLI `abel test` 新增 fixture lifecycle 第一片：自动调用测试文件中的 `setup` / `teardown`，并复用同一个 Interpreter 运行上下文。
338. CLI 回归覆盖 setup 失败、teardown 失败、main 失败后仍执行 teardown、错误 fixture 签名诊断，以及 main exit code 与 teardown 诊断同时保留。
339. Package manager 新增 `publishPackageToLocalRegistry` 与 CLI `abel package publish [--overwrite] <project-dir> <registry-dir>`，将包发布到本地 registry 的 `<name>/<version>`，并跳过源包 `.abel` 缓存。
340. package/CLI 回归覆盖本地 registry 发布、重复版本拒绝、`--overwrite` 覆盖、发布后 `abel add registry` 与 `abel run` 消费闭环。
341. Package manager 新增本地 registry index 数据面：`PackageRegistryEntry` / `PackageRegistryIndexResult`、`scanLocalPackageRegistry`、`writeLocalPackageRegistryIndex`、`checkLocalPackageRegistryIndex` 与 `.abel-registry.json`。
342. `abel package publish` 发布成功后自动刷新 registry index；CLI 新增 `abel package registry index <registry-dir>`、`abel package registry check <registry-dir>`、`abel package registry list <registry-dir>`。
343. package/CLI 回归覆盖 registry index 写入、list 输出、check 通过、手动改 registry 后 stale index 诊断、index 重建，以及目录名与 manifest name/version 不一致诊断。
344. README / TUTORIAL_zh.md / CODEX.md 同步本地 registry index 操作闭环与边界：这是本地目录 registry 的索引第一片，不是远程 registry、网络下载或完整 solver。
345. registry dependency resolver 现在在 `.abel-registry.json` 存在时先运行 index 校验并从 index entries 选择满足 SemVer requirement 的最高版本；stale/malformed index 会阻断 lockfile 解析。
346. package QTest 增加 indexed registry resolution 回归：有索引时选择索引中的最高满足版本，手动新增 registry 版本造成索引 stale 后 `resolvePackageLock` 拒绝继续解析。
347. README / TUTORIAL_zh.md / CODEX.md 同步 resolver 消费 registry index 的操作边界：先重建 stale index，不绕过索引；没有 index 的早期 registry 才 fallback scan。
348. Package manager 新增 `packageRegistryMirrorCacheDir` 与 `file://` registry URI 解析路径：本机 `file://` registry 会先复制/刷新到根项目 `.abel/cache/registries/<sanitized-uri>`，再从镜像目录消费 `.abel-registry.json` 或目录扫描。
349. `abel add registry` 支持把 `file://` registry URI 原样写入 manifest，并在刷新 lockfile 时验证/镜像该 registry；lockfile `source` 保留 URI，`resolvedPath` 仍指向 `.abel/cache/packages/<name>/<version>`。
350. package 回归覆盖 manifest 直接声明 `file://` registry URI、`abel add registry` 写入 URI、镜像目录存在、最高满足 SemVer 版本选择、package cache 和 lockfile source/resolvedPath。
351. README / TUTORIAL_zh.md / CODEX.md 同步 `file://` registry mirror-cache 第一片：这是后续远程/download cache 的接口形状，不等于 HTTP/network registry 或完整 solver。
352. BuiltinRegistry 新增 std.io/std.path 文件路径第一片：`read_text`、`write_text`、`read_lines`、`write_lines`、`path_exists`、`path_is_file`、`path_is_dir`、`mkdirs`。
353. TypeChecker 增加文件路径 builtin 静态规则：路径参数必须是 `str`，`write_text` 内容必须是 `str`，`write_lines` 第二参必须是 `vector<str>`，direct call 与 pipe call 返回类型保持一致。
354. Interpreter 通过 BuiltinRegistry 执行 UTF-8 文本读写、按行读写、路径判断与递归建目录；运行期文件打开/写入/建目录失败产生结构化诊断。
355. builtin/typechecker/interpreter 回归覆盖文件路径 builtin 注册、成功读写、pipe 形态、静态误用拒绝和运行期文件错误诊断；README / TUTORIAL_zh.md / CODEX.md 同步普通文件路径能力不再必须 backend。
356. BuiltinRegistry 新增 std.io/std.path/std.env 第二片：`append_text`、`copy_file`、`move_path`、`remove_path`、`path_join`、`path_dirname`、`path_basename`、`path_ext`、`path_absolute`、`path_clean`、`current_dir`、`env_exists`、`env_get`。
357. TypeChecker 增加第二片文件/路径/环境 builtin 静态规则：追加/复制/移动/path_join 的第二参数必须是 `str`，`current_dir()` 必须零参，路径查询/环境查询返回类型和 direct/pipe 调用保持一致。
358. Interpreter 通过 BuiltinRegistry 执行 UTF-8 追加写入、文件复制、路径移动/删除、路径拼接拆分、当前目录查询和环境变量读取；运行期失败产生 E044x 结构化诊断。
359. builtin/typechecker/interpreter 回归覆盖第二片文件/路径/环境 builtin 注册、运行结果、pipe 形态和静态误用拒绝；README / TUTORIAL_zh.md / CODEX.md 同步普通文件、路径、环境能力边界。
360. BuiltinRegistry 新增 std.char 第一片：`char_code`、`char_from_code`、`char_is_digit`、`char_is_letter`、`char_is_alnum`、`char_is_space`、`char_is_upper`、`char_is_lower`、`char_upper`、`char_lower`、`char_to_str`。
361. BuiltinRegistry 新增 std.any 第一片：`any_type`、`any_is`、`any_is_bool`、`any_is_int`、`any_is_double`、`any_is_char`、`any_is_str`、`any_is_vector`、`any_is_pointer`；不提供 `any_unwrap` 动态逃逸口，避免破坏 check/run 类型一致性。
362. TypeChecker 增加 std.char/std.any direct 与 pipe 静态规则；builtin/typechecker/interpreter 回归覆盖注册、运行结果、pipe 形态、静态误用拒绝与 `char_from_code` 越界运行期诊断；README / TUTORIAL_zh.md / CODEX.md 同步 std.char/std.any 使用边界。
363. ResourceNode 新增 `platform`、`compiler`、`compilerVersion`、`cxxStandard`、`abelAbi` 字段与当前 runtime 查询函数；load 阶段在 QPluginLoader 前逐项检查 Qt version、kit、platform、compiler、compiler version、C++ standard 与 Abel ABI。
364. Package backendArtifacts 对新增兼容字段自动填当前 runtime 默认值，普通用户继续只需写 backendId/path/symbols/build；backend artifact cache sidecar 升级到 formatVersion 2，并把平台/编译器/C++/Abel ABI 字段纳入 stale 判定。
365. backend/package 回归覆盖专家 ResourceNode JSON 新字段、foreign compatibility 字符串 parse 不拒绝但 load 拒绝、package manifest 默认填充、cache metadata 写入与 compatibility metadata stale 时回退源 artifact；README / TUTORIAL_zh.md / CODEX.md 同步 ResourceNode 专家字段与普通 package 路径。
366. Parser consume/recovery 规则从“缺失 token 时直接吞下一个 token”改为按预期 token 与发现 token 决定是否合成缺失 token；缺分号、缺右括号、缺右花括号等高频错误不再破坏后续 AST 主干。
367. Parser block-level recovery 增加声明/语句同步点：普通 block 遇到疑似顶层声明会停止并把控制权交回顶层 parser，backend/struct 成员场景仍保留合法 `fn` 成员解析。
368. 增加 parser/typechecker 回归：缺语句分号后保留 var decl 与 return；未闭合函数体后保留下一个顶层 `main`；parser 根因不会继续追加 missing-return 噪音。
369. Lexer/Parser 增加 `operator` 关键字与 `fn R operator <symbol>(A a, B b)` 声明解析，`FunctionDeclNode` 记录 `isOperator/operatorSymbol`，函数名内部归一为 `operator +` 等形式。
370. TypeChecker 增加用户自定义二元 operator 分发：内建 numeric、字符串拼接、基础相等比较仍优先；非内建 operand 或无内建比较时查找可见 operator 函数，并按两个参数签名检查左右操作数。
371. TypeChecker 对 operator 声明执行第一片合法性检查：必须恰好两个参数，不能 variadic，不能是 debt，且当前切片禁止 mutable reference 参数，避免普通二元表达式无法提供真实 lvalue 时 check/run 分裂。
372. Interpreter 增加用户二元 operator 运行时分发：非 numeric 或非内建 equality case 会调用同一可见 operator 函数，并在 runtime stack 中加入 `operator <symbol>` 调用帧。
373. Parser/TypeChecker/Interpreter 回归覆盖 `Point operator +`、`Point operator ==` 的 check/run 正例，以及 operator arity 错误和 operand 类型不匹配负例。
374. TypeChecker / Interpreter 的函数收集对同一 package/module 下的 operator 函数放开多签名共存；普通同名函数仍按旧规则拒绝重复。
375. 用户二元 operator 调用增加 overload-set 候选选择：精确值参数优先，其次 const reference 精确绑定，再其次可转换参数；无匹配时报 no matching，同分多匹配时报 ambiguous。
376. duplicate operator 判定从“同名即重复”收窄为“同一 operator 符号 + 相同参数类型签名重复”，允许 `Point operator +` 与 `Size operator +` 并存。
377. TypeChecker / Interpreter 回归覆盖同一 `operator +` 的 `Point+Point` 与 `Size+Size` 多签名正例、重复签名负例、`any` 候选导致的歧义负例。
378. TypeChecker / Interpreter 的普通顶层函数收集放开同名多签名共存，并把 duplicate 判定收窄到同一 package/module 下相同参数类型签名；`main/setup/teardown` 根入口优先选择零参签名。
379. 普通函数 direct call、pipe call 与 module-qualified call 进入 overload-set 候选选择；第一片候选评分与 operator overload-set 保持同构：精确值参数优先，其次 const/reference 精确绑定，再其次可转换参数，variadic any... 作为较弱候选。
380. Interpreter 增加表达式实参预备层，在 overload 选择前同时取得值、可选 location、readonly 与 SourceSpan；选中 mutable/reference 参数后继续按真实 location 绑定，避免 check/run 在引用参数上分裂。
381. TypeChecker / Interpreter 回归覆盖普通函数多签名正例、引用参数写回、pipe 形态、多签名重复拒绝、无匹配 overload 与 `any` 候选歧义。
382. TypeChecker / Interpreter 的 struct 收集从单构造/单方法映射升级为 constructor list 与 method overload-set，duplicate 判定收窄到同一 struct 内相同参数类型签名。
383. struct constructor call 与 method call 进入 overload-set 候选选择；多个候选时使用和普通函数/二元 operator 同构的第一片评分，单候选保留旧参数转换诊断。
384. Interpreter 增加 struct 方法/构造 prepared-call 路径，在 overload 选择前只求值一次实参，并保留 reference 参数真实 location、readonly 与 SourceSpan，避免副作用重复与 check/run 分裂。
385. struct overload 仍保持 private 构造/方法访问控制、const receiver 规则、零参 init 默认构造与无 init positional construction 兼容。
386. TypeChecker / Interpreter 回归覆盖 constructor overload、method overload、引用参数写回、实参只求值一次、duplicate constructor/method、no matching method/constructor 与 `any` 候选歧义。
```
...
- 2026-06-23 v1 `std.test` 断言 builtin 第一块落地：
  - 构建与定向 builtin/typechecker/interpreter/CLI 回归测试在 4GB 虚拟内存上限下通过：
    ...
    输出 4/4 tests passed。
- 2026-06-24 v1 `std.test` 第二块落地：
  - 构建与定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
    输出 3/3 tests passed。
- 2026-06-24 v1 `abel test --filter` 第一块落地：
  - 构建与定向 CLI 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "cli_multifile_project" -j1'
    输出 1/1 tests passed。
- 2026-06-24 v1 `abel test --report-json` 第一块落地：
  - 构建与定向 CLI 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "cli_multifile_project" -j1'
    输出 1/1 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
...
- 2026-06-24 v1 `abel test --report-junit` 第一块落地：
  - 构建与定向 CLI 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "cli_multifile_project" -j1'
    输出 1/1 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 14/14 tests passed，git diff --check 无输出。
- 2026-06-24 v1 `abel test --expect-fail` 第一块落地：
  - 构建与定向 CLI 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "cli_multifile_project" -j1'
    输出 1/1 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 14/14 tests passed，git diff --check 无输出。
- 2026-06-24 v1 `abel test` fixture lifecycle 第一块落地：
  - 构建与定向 CLI 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "cli_multifile_project" -j1'
    输出 1/1 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 14/14 tests passed，git diff --check 无输出。
- 2026-06-24 v1 `abel package publish` 本地 registry 发布第一块落地：
  - 构建与定向 package/CLI publish 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "package_manifest|cli_package_publish" -j1'
    输出 2/2 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 `abel package registry index/check/list` 本地 registry 索引第一块落地：
  - 构建与定向 package/CLI publish 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "package_manifest|cli_package_publish" -j1'
    输出 2/2 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 indexed registry resolution 第一块落地：
  - 构建与定向 package/CLI publish 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "package_manifest|cli_package_publish" -j1'
    输出 2/2 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
...
- 2026-06-24 v1 std.io scan 第一块落地：
  ...
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 14/14 tests passed，git diff --check 无输出。
- 2026-06-24 v1 std.math builtin 第一块落地：
  - 构建通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build'
  - 定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
  ...
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 14/14 tests passed，git diff --check 无输出。
- 2026-06-24 v1 std.str 扩展方法第一块落地：
  - 构建通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build'
  - 定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
  ...
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 14/14 tests passed，git diff --check 无输出。
- 2026-06-24 v1 std.math 第二块落地：
  - 构建与定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
    输出 3/3 tests passed。
- 2026-06-24 v1 std.vec builtin algorithms 第二块落地：
  - 构建与定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
    输出 3/3 tests passed。
- 2026-06-24 v1 std.str 第二块落地：
  - 构建与定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
    输出 3/3 tests passed。
- 2026-06-24 v1 std.vec 第三块落地：
  - 构建与定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
    输出 3/3 tests passed。
- 2026-06-24 v1 std.char/std.any 第一块落地：
  - 构建通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build'
  - 定向 builtin/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "builtin|typechecker|interpreter" -j1'
  ...
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 backend compatibility metadata 第一块落地：
  - 构建与定向 backend/package/SDK 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "backend|package_manifest" -j1'
    输出 6/6 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 Parser 诊断恢复第一块落地：
  - 构建与定向 parser/typechecker 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "parser|typechecker" -j1'
    输出 2/2 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 `file://` registry mirror-cache 第一块落地：
  - 构建与定向 package_manifest 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R package_manifest -j1'
    输出 1/1 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 用户自定义二元 operator 第一块落地：
  - 构建与定向 parser/typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "parser|typechecker|interpreter" -j1'
    输出 3/3 tests passed。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
- 2026-06-24 v1 用户自定义二元 operator overload-set 第一块落地：
  - 构建与定向 typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "typechecker|interpreter" -j1'
    输出 2/2 tests passed。
- 2026-06-24 v1 普通函数 overload-set 第一块落地：
  - 构建通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build'
  - 构建与定向 typechecker/interpreter 回归测试在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -R "typechecker|interpreter" -j1'
    输出 2/2 tests passed。
  - 第一轮全量回归暴露两个兼容断言：单签名函数仍应保留转换诊断，import ambiguity 测试仍期待旧文案。已修正为“单签名走旧路径，多签名才报 no matching overload；相同签名的跨 import 歧义保留 ambiguous across imported modules 文案”。
  - 构建、全套 QTest/CTest 与 whitespace 检查在 4GB 虚拟内存上限下通过：
    /bin/bash -lc 'ulimit -v 4194304; /home/tnuzy/Qt/Tools/CMake/bin/cmake --build build && /home/tnuzy/Qt/Tools/CMake/bin/ctest --test-dir build --output-on-failure -j1 && git diff --check'
    输出 15/15 tests passed，git diff --check 无输出。
```
...
8. 当前诊断去污染已覆盖 TypeChecker 表达式/语句级 unknown 传播、definite-return 第一片与 Parser 缺分号/未闭合 block 恢复第一片；resolver、多文件符号恢复、更完整 parser recovery、更精细控制流返回分析仍需后续大块继续做。
9. 当前 package/init/update/add/remove/build/package publish/registry index/graph/cache/install-sdk 只是 v1 包管理与 backend SDK 入口前几片：resolver 支持本地 path dependency、本地 registry dependency、`file://` registry URI mirror-cache、SemVer requirement 第一片、同名包多解析冲突诊断第一片与 `.abel-registry.json` 索引消费第一片，registry publish/cache/index/mirror 只是把本地目录或 file URI 镜像复制到 `.abel/cache/registries`、生成/消费 `.abel-registry.json` 或复制到 `.abel/cache/packages`，backend artifact 自动构建只支持 CMake 第一片，安装版 SDK 只是第一片；backend cache sidecar 已覆盖源 artifact 路径/size/mtime、ResourceNode 字段、platform/compiler/C++/Abel ABI 字符串与 symbols 匹配，但还没有 HTTP/network registry、完整 semver solver、成熟网络 download cache、完整 SDK 成熟化、二进制内容 ABI hash/semver 级校验或完整版本化缓存失效。
...
11. 当前 ResourceNode compatibility gate 比较 ResourceNode 声明的 Qt version、kit、platform、compiler、compilerVersion、cxxStandard 与 Abel ABI 字符串，能提前挡住明显错 kit、错 Qt、错平台或错编译器资源，但不等价于完整二进制 ABI hash、libstdc++ ABI 细分、semver 或二进制内容校验。
12. 当前 `abel test` 已有项目级测试入口、按测试文件相对路径 substring 的 `--filter` 第一片、`--expect-fail` 第一片、`--report-json` 机器可读报告第一片、`--report-junit` CI 报告第一片与 `setup`/`teardown` fixture lifecycle 第一片；还没有并行调度、覆盖率、TAP 输出或更成熟 report 格式。
13. 当前 `std.test` 已覆盖基础断言、相等/不等断言与浮点近似断言 `test_close`，CLI 层已有 setup/teardown fixture lifecycle；但还没有深度自定义比较、集合 diff、标准库层 fixture API 或成熟测试报告。
...
16. 当前 struct public/private 是成员级第一片：已覆盖字段、构造、方法与默认构造访问控制；struct constructor/method overload-set 已完成第一片，但还没有 per-symbol module private/public、friend、protected、nested type、private static 成员或更成熟封装诊断。
17. 当前 std.vec 已覆盖基础方法、排序/查找、第二批常用算法、contains/count/extend/slice，但还没有完整 iterator、stable erase 语义文档、用户 comparator、stable algorithms、map/filter/reduce、view 或成熟容器标准库分层。
18. 当前 std.io/std.path/std.env 仍是早期切片：`scan(&x, ...)` 支持 whitespace token 输入并写入基础标量、str、any；文件/路径/环境 builtin 支持 UTF-8 整文件读写、追加、按行读写、路径存在/文件/目录判断、复制、移动、删除、递归建目录、路径拼接拆分、当前目录与环境变量读取；但还没有格式化输入、流式 IO、文件句柄对象、错误返回对象、权限/符号链接细策略、成熟 std.io module 分层或可注入输入源 API。
19. 当前 std.math 已覆盖常用 numeric builtin 与三角/指数/对数/gcd/lcm 第二片，但还没有随机数、溢出策略文档、NaN/Inf 诊断策略、角度单位辅助、复数/向量数学或成熟 module 分层。
20. 当前 std.str 已覆盖基础查找/切片/替换、第一批扩展方法、join 与基础 parse 系列，但还没有正则、locale-aware case folding、format、streaming、错误返回对象或成熟 module 分层。
21. 当前 std.char/std.any 只是第一片：std.char 覆盖 Qt `QChar` 常用分类、大小写、code unit 与单字符字符串转换；std.any 覆盖 runtime type 名称和常用谓词，但 `any` 真实取值仍必须 `cast<T>(any)`，还没有 Result/error 对象、模式匹配、反射 API 或成熟 module 分层。
22. 当前用户自定义 operator 只是二元 overload-set 第一片：支持同一可见 operator 符号下多个二元签名候选和简单评分/歧义诊断，但不支持 `operator()` / `operator[]` / `operator<>`，不支持 unary operator overload，不支持 member operator，不支持 mutable-reference operator 参数，也不是完整 C++ overload ranking；内建 numeric/str/basic equality 仍优先于用户 operator。
23. 当前普通函数 overload-set 只是顶层普通函数第一片：支持 direct/pipe/module-qualified call 与简单评分/歧义诊断；struct constructor/method overload-set 已作为独立第一片支持，但普通函数系统仍不支持 backend overload、返回类型 overload、默认参数、模板函数 overload 或完整 C++ overload ranking；单签名函数仍保留原有转换诊断路径。
24. 当前 struct constructor/method overload-set 只是第一片：支持同一 struct 内多签名 `init(...)` 与同名方法、duplicate/no-match/ambiguous 诊断、private/const receiver 约束与引用参数真实 location 绑定；但不支持返回类型 overload、默认参数、模板方法/构造、完整 C++ overload ranking、backend overload 或 prvalue struct 临时对象方法调用的完整生命周期模型。
```
...
3c58595 builtins: add scan input
1c3daf9 builtins: add math functions
9743375 builtins: expand string methods
6322e60 builtins: expand math functions
fa9e51a builtins: expand vector algorithms
dacc1f3 builtins: expand string parsing
8047435 builtins: expand vector utilities
c08ba41 builtins: add floating test assertions
074f491 cli: filter project tests
96d4762 cli: write test json reports
738fa55 cli: write test junit reports
3ff20df cli: support expected failing tests
0611881 cli: add test fixtures
74df116 package: publish to local registry
9d020dd package: index local registry
7f1f36d package: consume registry index
fd242b3 builtins: add file path helpers
c006f04 builtins: expand file path env helpers
60c2ef7 builtins: add char any helpers
d71716f backend: gate resource compatibility metadata
95c6cb8 parser: recover from common syntax errors
6a0666c package: mirror file registry uris
def549d language: add binary operator overloads
f42c424 language: resolve operator overload sets
115354e language: add function overload resolution
