...
本轮实现任务：v1.2 Dynamic Waterworks complete。新增 anytuple `[[...]]` 与 strmap `[{"k" = v}]` AST/parser/typechecker/interpreter 主路径：literal 返回 `any`，runtime 创建 `AbelDynamicObject` kind `tuple` / `strmap`，支持 `[]` / `[]=` / `any_type` / `any_debug` / dynamic identity equality；strmap duplicate key 在 check 阶段诊断，tuple bad index/out-of-range 与 strmap bad key/missing key 运行期稳定诊断。Pipe RHS 含 `_` 且不是旧式 callable insertion 时走 general expression pipe context，已支持 `_ + _`、`[[_, _ + 1]]`、`[{"k" = _}]`、`_["k"]`、`_["k"] = ...` 和 `_ = ...` lvalue 写回；旧 named/default/spread callable pipe 保持原路径。本轮补齐 v1.2 complete 验收证据：complex matrix 扩展到 27 个 heavy programs 与 14 个 corner programs，其中 v1.2 专项覆盖 nested route、struct payload、vector<any> rows、projection、callback-in-tuple、writeback、set new key、identity/debug、type dispatch、旧/新 pipe 混用，以及 tuple/strmap 关键 runtime/check diagnostics；CLI multifile project smoke 覆盖模块/alias 下的 waterworks check/run；installed SDK fixture 覆盖 dynamic tuple/strmap 经 AbelValue/any... 进入插件边界。
```
