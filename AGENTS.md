...
template syntax retired/reserved；不再作为 v1/v1.1-H 承诺能力
...
template 路线从 v1.1-H 起整体退休。历史实现过的最简无约束 template 不再作为承诺能力扩展；后续目标是迁移测试与文档，最终让 parser/typechecker 对 template 入口给出稳定 retired/reserved 诊断，而不是继续维护一条弱化版 C++ 泛型路线。
...
template 全路线退休：不再 legacy/frozen 地保留为未来选项，而是迁移到 retired/reserved 诊断。
```
...
3. 现有 template 测试改为 retired/reserved 诊断测试，或在删除实现前短期隔离为 legacy coverage。
4. Parser / TypeChecker 最终对 template 入口给稳定 retired/reserved 诊断。
...
```text
不保留 template 路线；最终给 retired/reserved 诊断。
不做 tuple<T...>。
...
```text
1. 文档宣布 template retired，不再 legacy/frozen 地作为未来能力保留。
2. Parser/TypeChecker 对 template 入口给稳定 retired/reserved 诊断，或短期有明确迁移测试计划。
...
```text
1. template retired 切除：文档删除推荐路线，Parser/TypeChecker 给 retired/reserved 诊断，测试从 template success 改为 retired diagnostic。
...
本轮实现任务：v1.1-H H3 隐式 dynamic cast 第一片已落地：TypeChecker 在非 ref 目标类型位置允许 `any -> T`，Interpreter 的 `convertOrError` 复用 dynamic cast，覆盖变量初始化、赋值、普通函数实参、return、vector 恢复和 backend 参数；失败保持 E0591 runtime diagnostic。
```
...
历史无约束 template 第一片曾落地，但新路线已决定 template 全路线 retired；当前已清掉 Abel 用户 template 的 AST 字段、TypeChecker 推导/实例化路径、Interpreter 运行路径和成功测试；显式类型实参和 retired generic type syntax 也只在 Parser 层诊断/恢复，不再进入 AST、TypeChecker 或 Interpreter。保留的尖括号语法只剩 core type / cast 语法：`vector<T>`、`func`、`cast<T>(x)`。
...
v1.1-H 路线已在 AGENTS.md 定死：template 全路线 retired；any/cast/operator/backend 成为 v1.2 主路线；TypeChecker 只守静态边界和显式 dynamic boundary，不追踪 any/tuple/map/object schema；backend/SDK 需要强化 AbelValue、AbelVariadicArgs、AbelRuntimeContext、AbelCallable、dynamic object、deep copy/equality/hash/debug/introspection；operator 能力改走 concrete overload + any dynamic dispatch + backend dynamic operator protocol；v1.2 any tuple literal `[[...]]` 与 str map literal `[{...}]` 暂定为 backend-backed dynamic literal lowering，并通过 operator [] / []= 访问，不新增 core tuple/map/object TypeKind，不做 tuple<T...> 或 schema inference。
...
template v1 最简无约束函数/struct/type alias 与 exact-shape 二元 operator template 历史上已落地，但现在全路线 retired；当前已完成实现清理与测试迁移，Parser 对 `template <type ...>`、非 core generic type syntax 和显式类型实参给稳定 retired/reserved 诊断，TypeChecker/Interpreter 不再保留 template bindings / instantiation / exact-shape operator template / explicit type args / generic type args 逻辑。
...
v1.1-H 状态：H1 template retired deep-clean 已完成；H2/H4 any introspection/debug 第一片已完成；H3/H4 explicit cast 第一片已完成（func exact cast、vector element-wise cast）；H3 隐式 dynamic cast 第一片已完成（init/assign/call/return/backend 参数）。下一步执行顺序是 AbelCallable / backend calls Abel func → SDK dynamic helper → operator dynamic dispatch → 安装版 SDK fixture 覆盖。
