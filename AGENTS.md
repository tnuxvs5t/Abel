...
LSP 当前是 Phase 3+ 体验边界：全量重算、AST/token 索引、基础补全、AST 大纲、diagnostics、hover/definition/workspace symbol、references/highlights、signature help、folding ranges、semantic tokens、AnalysisIndex-backed rename 已可用；Analyzer 已切到 TypeChecker opt-in AnalysisIndex，并覆盖变量/调用/字段/方法/构造/backend 的第一批精确 binding；简单 struct receiver 的字段/方法补全已可用。下一步是补 module/use/type/enum binding、链式/调用 receiver 成员补全、prepareRename/code action/inlay hints、缓存/增量/取消，继续向 CLion 级体验推进。
