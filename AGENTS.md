...
```text
v0 后续：
1. 若继续开发，优先从延期项中选择一个大块：scan、完整 const 矩阵、backend binder 类型矩阵、struct 高级项或用户自定义 operator。
2. 每个后续大块仍需保持 Git 审计、4GB 测试限制、AGENTS.md 强制区更新。
3. 发布后若需要更强访问控制，应将 GitHub 仓库改 private；若保持 public，则依靠 LICENSE/著作权声明保护使用权边界。
```
...
待本次 v1-complete-scope-docs 提交后追加 commit hash。
