...
4. 语言语义下一批：在 package 多文件/依赖库源码合并、跨包 export enforcement、package/module-aware function/struct/backend resolution、module/use 可见性、限定函数/backend/struct/type、import alias 与导入冲突诊断第一片之上推进显式 re-export、多文件 source map API、更细 per-span label、更多 const/引用/生命周期边界、泛型/模板/接口实际语义，以及 parser/resolver 级诊断恢复。
