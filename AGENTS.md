...
本轮实现任务：删除除 AGENTS.md 外的 Markdown 累赘并提交；AGENTS.md 保持唯一当前路线手册。
```
...
历史无约束 template 第一片曾落地，但新路线已决定 template 全路线 retired；后续应迁移文档和测试，让 template 入口成为稳定 retired/reserved 诊断。
历史 exact-shape 二元 operator template 曾落地，但新路线已放弃 template operator；operator 能力改走 concrete overload + any dynamic dispatch + backend dynamic operator protocol。
...
template v1 最简无约束函数/struct/type alias 与 exact-shape 二元 operator template 历史上已落地，但现在全路线 retired；后续目标是删除推荐文档、迁移测试、给 parser/typechecker 稳定 retired/reserved 诊断。
...
v1.1-H 状态：当前为规划落地起点。执行顺序是 template retired → any 动态核心 → cast/隐式 dynamic cast → func-as-any / AbelCallable → SDK dynamic helper → operator dynamic dispatch → 安装版 SDK fixture 覆盖。
