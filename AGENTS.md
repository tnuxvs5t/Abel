...
2. 下一批语言语义债建议聚焦：诊断去污染、`__FILE__` / `__LINE__` / `__COLUMN__` 或等价 source_location 内建、更多 const/引用/生命周期边界、泛型/模板/接口实际语义。
...
待本次 v1-vector-struct-default-construction 提交后追加 commit hash。
