...
12. 当前 `abel test` 已有项目级测试入口、按测试文件相对路径 substring 的 `--filter` 第一片、`--expect-fail` 第一片、`--report-json` 机器可读报告第一片与 `--report-junit` CI 报告第一片；还没有并行调度、测试 fixture 生命周期、覆盖率、TAP 输出或更成熟 report 格式。
13. 当前 `std.test` 已覆盖基础断言、相等/不等断言与浮点近似断言 `test_close`，但还没有深度自定义比较、集合 diff、fixture API 或成熟测试报告。
