...
23. 当前普通函数 overload-set 只是顶层普通函数第一片：支持 direct/pipe/module-qualified call 与简单评分/歧义诊断，但不支持 method/constructor/backend overload、返回类型 overload、默认参数、模板函数 overload 或完整 C++ overload ranking；单签名函数仍保留原有转换诊断路径。
```
