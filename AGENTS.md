...
2. 包管理下一批大块：registry/semver/download cache、backend build spec 成熟化、安装版 SDK 成熟化与版本化缓存失效，而不是停留在本地 path dependency、lockfile consumption、缓存复制与 CMake 自动构建第一片。
3. Backend/SDK 下一批大块：variadic any backend binder、明确 pointer/reference 暴露矩阵、ABI/Qt kit 缓存失效策略。
...
9. 当前 package/init/update/add/remove/build/graph/cache/install-sdk 只是 v1 包管理与 backend SDK 入口前几片：resolver 只支持本地 path dependency，backend artifact 自动构建只支持 CMake 第一片，安装版 SDK 只是第一片；还没有 registry、semver solver、download cache、完整 SDK 成熟化、ABI 校验或版本化缓存失效。
...
待本次 v1-backend-artifact-auto-build 提交后追加 commit hash。
