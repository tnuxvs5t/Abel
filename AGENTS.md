...
本轮实现任务：继续 v1.1-a Structured Calls 收口；在 constructor / method / backend / static-qualified structured call 后，补齐普通函数 pipe RHS structured call（named/default/spread + `_` holes）的 TypeChecker/Interpreter/测试闭环。
```
...
v1.1-a 剩余：backend/constructor 路径的 pipe hole 归一化仍未完成；receiver-hole 第一片当前限制为单 `_`，尚未开放 `_.method(_)` 这类多 hole / mutable receiver 组合；builtin pipe RHS spread 暂未开放，builtin method 与 function value call 继续保持 positional-only 边界；任何后续推进都必须继续走 TypeChecker/Interpreter/测试三件套闭环。
```
