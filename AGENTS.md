...
4. 语言语义下一批：在 package 多文件合并第一片之上推进真正 module/use 语义、依赖包源码编译、限定名/导出可见性、多文件 source map API、更多 const/引用/生命周期边界、泛型/模板/接口实际语义，以及 parser/resolver 级诊断恢复。
...
7. 当前 `__FILE__` / `__LINE__` / `__COLUMN__` 与 CLI sourceLine excerpt 已能在 package 多文件合并时保留各文件 AST SourceSpan；但 `module/use` 仍只是语法与合并入口第一片，不代表真正 import 过滤、模块可见性、依赖包源码编译或限定名查找已经完成。合成 AST 节点的 sourceLine 当前沿用首 token 行，多行 span 的完整 excerpt 仍是后续工作。
...
待本次 package multi-file/module 提交后追加 commit hash。
