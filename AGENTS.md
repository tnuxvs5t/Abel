...
member operator
operator overload set / 同名多签名重载解析
...
22. 当前用户自定义 operator 只是二元第一片：只支持单个可见 operator 函数名查找，不支持 overload set / 多签名重载解析，不支持 `operator()` / `operator[]` / `operator<>`，不支持 unary operator overload，不支持 member operator，不支持 mutable-reference operator 参数；内建 numeric/str/basic equality 仍优先于用户 operator。
```
