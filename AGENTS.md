...
本轮实现任务：v1.1-H H1 deep-clean 已落地：清掉 Abel 用户 template 的 AST / Parser 成功入口 / TypeChecker 实例化推导 / Interpreter 运行路径，并把显式类型实参和 retired generic type syntax 下沉为 Parser 诊断，不再把 template/generic 状态带入 AST、TypeChecker 或 Interpreter，为后续 any-based typecheck 重做打干净基础。
```
...
v1.1-H 状态：H1 template retired deep-clean 已完成；下一步执行顺序是 any 动态核心 → cast/隐式 dynamic cast → func-as-any / AbelCallable → SDK dynamic helper → operator dynamic dispatch → 安装版 SDK fixture 覆盖。
