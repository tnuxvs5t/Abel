...
本轮实现任务：v1.1-H H2/H4 第一片已落地：any 动态 introspection 公开 `any_debug`、`any_is_struct`、`any_is_func`，TypeChecker / BuiltinRegistry / Interpreter 测试覆盖 struct value 与 func value 进入 any 后的 type/debug/is 查询，为后续 cast<func>、AbelCallable 和 backend callable 调用铺路。
```
...
v1.1-H 状态：H1 template retired deep-clean 已完成；H2/H4 any introspection/debug 第一片已完成；下一步执行顺序是 cast/隐式 dynamic cast → func-as-any cast 恢复 / AbelCallable → SDK dynamic helper → operator dynamic dispatch → 安装版 SDK fixture 覆盖。
