...
2. 第一批语言语义债建议聚焦：check/run 一致性、lvalue/prvalue receiver、vector<struct>.resize 默认构造、numeric conversion、source location/stack trace。
...
5. v1 source location 必须贯穿 AST、runtime frame、backend call context；只传入口文件路径不满足 `__FILE__` / `__LINE__` 需求。
```
...
待本次 v1-diagnostics-semantics-docs 提交后追加 commit hash。
