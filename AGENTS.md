...
```text
Stage 14b：`cast<T>(any)`、pipe 表达式、CLI ResourceNode 运行入口已完成；继续 v0 剩余差距审计与收口。
```
...
3. BuiltinRegistry 尚未接入 scan；`to_str` 尚未支持用户自定义重载，只覆盖内建 stringify。
...
2. 优先审计 `scan`、用户自定义 `to_str(T)`、const 引用/指针、backend binder 类型矩阵、struct 高级项是否属于 v0 必须完成。
...
3. 用户自定义 `to_str(T)` 尚未接入 build_string 的可见函数查找；当前 stringify 只覆盖内建类型、any、vector、pointer/nullptr。
4. `scan` 是否进入 v0 仍需最终裁定；AGENTS.md 允许先留接口位置，但若目标定义为完整 v0，需要明确延期理由。
...
待本次 Stage 14b expression-resource-run 提交后追加 commit hash。
