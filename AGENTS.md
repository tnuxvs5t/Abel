...
本轮实现任务：按当前 v1.1-a/b 边界更新 README / TUTORIAL_zh，并同步 abel-engineering skill 的语法参考与 bundled SDK 二进制资产。
```
...
README / TUTORIAL_zh 已按当前 v1.1-a/b 边界更新：Structured Calls 明确为静态归一化调用糖，function value / builtin method positional-only 边界、limited spread 约束、Backend Carries Complexity 路线和 SDK helper 使用方式已写入；abel-engineering skill 的语法参考与 bundled SDK 安装树已同步到当前本地构建。
```
